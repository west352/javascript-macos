...
const Preferences: React.FC<PreferencesProps> = (
    ...
    }, [preferencesState, close, open]);

    const handleSliderChange = useCallback(
        (newInput: number, title: string): void => {
            switch (title) {
                case CHANGE_DOCK_ICON_SIZE: {
                    setDockConfig({ ...dockConfig, iconSize: newInput });
                    break;
                }
                case CHANGE_DOCK_BIG_ICON_SIZE: {
                    setDockConfig({ ...dockConfig, bigIconSize: newInput });
                    break;
                }
                case CHANGE_DOCK_DISTANCE_BETWEEN_ICONS: {
                    setDockConfig({ ...dockConfig, distanceBetweenIcons: newInput });
                    break;
                }
                case CHANGE_DOCK_DISTANCE_TO_SCREEN_EDGE: {
                    setDockConfig({ ...dockConfig, distanceToScreenEdge: newInput });
                    break;
                }
            }
        },
    ...
    const sliderJSXElements: JSX.Element[] = sliders.map((slider, index) => {
        return (
            <div key={index + slider.maxValue} className="options">
                ...
                <input
                    ...
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                        handleSliderChange(parseInt(e.target.value), slider.title)
                    }
                />
